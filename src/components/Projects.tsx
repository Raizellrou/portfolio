...
const projects = [
  {
    title: "Project One",
    description:
      "A brief description of your first project. What problem does it solve? What makes it interesting?",
        ...
        <div className="space-y-8">
          {projects.map((project, i) => (
            <motion.article
              ...
              className="group rounded-2xl border border-border bg-card p-6 md:p-8 hover:border-foreground/10 transition-colors"
            >
              <div className="mb-4">
                <div className="flex flex-wrap gap-2 mb-3">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs font-mono px-2.5 py-1 rounded-full bg-accent/5 text-accent"
                    >
                      {tag}
                    </span>
                  ))}
